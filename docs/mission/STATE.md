...
- `ruff check backend` clean; `pytest backend/tests` → **343 passed** (unit) + 1 browser journey (43 steps) (API round trips, roles,
...
| One complete, reopenable end-to-end journey | VERIFIED_LOCAL | Committed 43-step browser journey against the real server with local auth and the offline guard (setup → design → brief with a creative direction → campaign table → generate → review per row → approve/reject → regenerate → export → save/reopen → members, tokens, second workspace, approver account); green on CI. |
...
3. Product follow-ups with evidence value: grammar families for extreme strips (728×90),
   multi-node job records and sessions.
