...
| Counterexample search: independent oracles (mask overlap, bounds, hard rules on final boxes, subject correlation) re-judge renders under seeded perturbations and list accepted renders they reject | VERIFIED_LOCAL (tool) | `backend/tools/find_counterexamples.py`; results in `docs/mission/results/`. |
