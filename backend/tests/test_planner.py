...
def _doc(tmp_path, long_copy: bool = False) -> tuple[DesignDocument, AssetStore]:
    ...
    doc.elements.append(
        Element(id="bg", kind="shape", name="Background", role="background",
                geometry=Geometry(0, 0, 1200, 628), z_index=0,
                shape={"type": "rect", "fill": "#2b4c7e"}, provenance=Provenance(origin="fixture"))
    )
    logo = store.put(Image.new("RGBA", (180, 80), (250, 250, 250, 255)), "logo")
    hero = store.put(Image.new("RGBA", (390, 430), (236, 88, 88, 255)), "hero")
    doc.elements.append(
        Element(id="logo", kind="image", name="Logo", role="logo",
                geometry=Geometry(960, 24, 180, 80), z_index=5, asset=logo, priority=1,
                provenance=Provenance(origin="fixture"))
    )
    doc.elements.append(
        Element(id="hero", kind="image", name="Hero", role="hero_image",
                geometry=Geometry(740, 120, 390, 430), z_index=3, asset=hero, priority=2,
                provenance=Provenance(origin="fixture"))
    )
    head = "MEGA CLEARANCE WEEKEND EVENT WITH EXTRA BONUS DISCOUNTS" if long_copy else "SUMMER SALE"
    doc.elements.append(
        Element(id="headline", kind="text", name="Headline", role="headline",
                geometry=Geometry(70, 50, 620, 120), z_index=4, priority=1,
                text=TextContent(runs=[TextRun(head, TextStyle(font_size=54, color="#ffffff",
                                                                weight="bold"))]),
                provenance=Provenance(origin="fixture"))
    )
    doc.elements.append(
        Element(id="sub", kind="text", name="Sub", role="subheadline",
                geometry=Geometry(70, 190, 620, 90), z_index=4, priority=2,
                text=TextContent(runs=[TextRun("Up to 50% off selected items",
                                               TextStyle(font_size=40, color="#ffffff"))]),
                provenance=Provenance(origin="fixture"))
    )
    doc.elements.append(
        Element(id="cta", kind="text", name="CTA", role="cta",
                geometry=Geometry(70, 305, 350, 95), z_index=4, priority=2,
                text=TextContent(runs=[TextRun("SHOP NOW", TextStyle(font_size=42, color="#ffd166",
                                                                     weight="bold"))]),
                provenance=Provenance(origin="fixture"))
    )
    doc.normalize_z()
...
def test_order_below_and_anchor_edge_constraints_are_honoured(tmp_path) -> None:
    ...
    doc.add_constraint(Constraint(id="c2", type="anchor_edge", elements=["logo"],
                                  params={"edge": "bottom"}))
    elements = elements_from_document(doc, store)
...
def test_scale_range_constraint_bounds_subject(tmp_path) -> None:
    doc, store = _doc(tmp_path)
    doc.add_constraint(Constraint(id="c3", type="scale_range", elements=["hero"],
                                  params={"min": 0.5, "max": 0.6}))
    elements = elements_from_document(doc, store)
    plan = plan_layout(doc, elements, (1080, 1920))
...
def test_content_pressure_gives_copy_room_in_small_formats(tmp_path) -> None:
    ...
    elements = elements_from_document(doc, store)
    fam = Family("square_text_top", Region(0.07, 0.15, 0.86, 0.34), Region(0.20, 0.52, 0.60, 0.44),
...
def test_variant_pipeline_with_constraint_planner_accepts_fixture(tmp_path) -> None:
    ...
    for target in [(1200, 628), (1080, 1920)]:
        res = generate_variant(doc, store, VariantBrief(*target, name="t"), planner="constraints",
                               quality_config=None)
