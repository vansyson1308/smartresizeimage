...
## Phase 2 policy modes (generative)

Current policy switches are controlled via `backend/app/config.py`:

- `GENERATIVE_BG_POLICY`
  - `BG_ONLY`: outpaint background-only editable regions.
  - `BG_PLUS_DECOR`: enables optional decor synthesis in decor zones.
- `GENERATIVE_DECOR_POLICY`
  - `OFF`: PR-C parity (no decor synthesis).
  - `BG_PLUS_DECOR`: apply decor synthesis with OCR-negative gate.

Quality-gate outcomes are exposed on each `CompositionResult`:
- `gates_passed`
- `fail_reasons`
- `used_fallback`
...
- `GENERATIVE_BG_SEED`
- `GENERATIVE_DECOR_SEED`
- policy values (`GENERATIVE_BG_POLICY`, `GENERATIVE_DECOR_POLICY`)
- `result.metadata["generative"]`, `result.metadata["decor"]`, and `result.metadata["quality_gates"]`.

These fields indicate backend usage, fallback reasons, and gate outcomes.

## Troubleshooting

- **Proxy/analytics errors when launching Gradio** (e.g. `httpx.ProxyError` from analytics threads):
  - set `GRADIO_ANALYTICS_ENABLED=false`
  - keep `AUTOBANNER_SHARE=true` in restricted localhost/proxy environments.
- **OpenCV/libGL missing** (`libGL.so.1`):
