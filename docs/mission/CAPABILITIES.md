...
| Layout grammar: families composed from arrangement × side/position × text share × logo corner × alignment (12 / 24 / 36 candidates per orientation) searched by the same scoring; traits recorded per plan | VERIFIED_LOCAL (mechanism) / ABLATED (effect, see EXPERIMENTS) | `design/grammar.py`, `AUTOBANNER_LAYOUT_GRAMMAR`; `test_grammar.py` (10). |
| Creative directions per run and per campaign row (emphasis, text side/position, alignment, arrangement, pinned family, mood) that narrow the candidates and report what a format cannot satisfy | VERIFIED_LOCAL | `grammar.parse_direction`/`apply_direction`, `direction` on briefs, rows and the CSV table, selector in the Variants view; `test_grammar.py`, browser journey step. |
| Subject/mascot integrity: uniform scale enforced (stretching is a critical failure) and an independent pixel oracle (correlation of the rendered region with the master asset) that sends occluded, recoloured or cropped subjects to review | VERIFIED_LOCAL | `variant.subject_integrity_checks`; `test_subject_integrity.py` (2). |
| Hideable copy on small sizes: explicitly permitted text is dropped least-important-first only when the stack cannot fit at minimum sizes; never headline/CTA/logo or keep_visible elements; recorded and flagged for review | VERIFIED_LOCAL | `planner._may_hide`, element panel checkbox; `test_planner.py`; `results/strips_2026-09-11.md` (three long-copy 728×90 cases failed → accepted with the permission). |
| Incremental refresh: after an edit, only variants whose shown elements, rules, fonts or canvas changed are re-rendered (layout kept); the rest are marked current with a reason | VERIFIED_LOCAL | `service.refresh_variants`, `POST .../variants/refresh`, review-view button; `test_refresh.py`. |
| Counterexample search: independent oracles (mask overlap, bounds, hard rules on final boxes, subject correlation) re-judge renders under seeded perturbations and list accepted renders they reject | VERIFIED_LOCAL | `backend/tools/find_counterexamples.py`, `test_counterexample_tool.py` (3); `results/counterexamples_2026-09-11.md`: 0 counterexamples on 420 renders, two findings turned into a fix and a follow-up. |
