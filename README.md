...
- **Grammar bố cục + chỉ dẫn sáng tạo**: ngoài 3 họ bố cục viết tay mỗi hướng, hệ thống ghép thêm 12–36 họ theo tổ hợp (chữ trái/phải/trên, tỷ lệ chữ, góc logo, căn lề) và chấm điểm như nhau; mỗi lần sinh hoặc mỗi dòng chiến dịch có thể kèm chỉ dẫn (`copy`, `subject`, `text-left`, `subject-top`, `center`…) — chỉ dẫn không đáp ứng được trên một kích thước sẽ được ghi rõ trong plan, không im lặng.
- **Giữ nguyên chủ thể/mascot**: chủ thể và logo phải giữ tỷ lệ (kéo giãn là lỗi nghiêm trọng) và giữ đúng pixel — vùng render được so với asset gốc bằng một oracle độc lập, bị che/đổi màu/cắt sẽ vào diện cần review kèm số đo.
- **Làm mới tăng dần**: sau khi sửa thiết kế, chỉ những biến thể bị ảnh hưởng bởi thay đổi mới được render lại (giữ bố cục), số còn lại được đánh dấu hiện hành kèm lý do.
