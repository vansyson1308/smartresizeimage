...
| Rules from correction history (H5): rejection + approved fix → reviewable proposal (`scale_range`, size-scaled `min_text_size`, `clear_space`) with evidence; one-click add; unparseable reasons reported | VERIFIED_LOCAL (mechanism) / EXPERIMENTAL (effect) | `design/corrections.py`, `project.record_rejection`, `GET .../learned` (`corrections`, `unresolved_corrections`), `POST .../learned/corrections/{i}/apply`; `test_corrections.py`, API test; rule-reviewer measurement in `EXPERIMENTS.md`. Rules are per project; brand-level carry-over across projects is PLANNED. |
